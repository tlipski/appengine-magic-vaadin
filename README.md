...
1. Use `def-appengine-servlet-app` instead of `def-appengine-app`, e.g.

    `(appengine-magic.core/def-appengine-servlet-app v1 (new clj_gae_vaadin.application_servlet))`

2. Enable real-time compilation of generated classes in dev mode (especially (proxy) and (fn) won't work correctly with Vaadin without it), e.g.
...
4. Use the correct servlet in `web.xml`.
